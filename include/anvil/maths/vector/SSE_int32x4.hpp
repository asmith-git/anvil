...
namespace anvil { namespace detail {
	...
	template<>
	struct OptimisedVector<int32_t, 3> {
		typedef int32_t T;
		enum { S = 3, optimised = 1 };
		typedef __m128i type;

		static ANVIL_STRONG_INLINE type ANVIL_VECTOR_CALL load(const Vector<T, S> a) throw() {
			return _mm_set_epi32(0, a.z, a.y, a.x);
		}
	...
	template<>
	struct OptimisedVector<int32_t, 4> {
		typedef int32_t T;
...
#ifndef ANVIL_AVX_2
	template<>
	struct OptimisedVector<int32_t, 8> {
		typedef float T;
		...
		static ANVIL_STRONG_INLINE type ANVIL_VECTOR_CALL load(const Vector<T, S> aVector) throw() {
			...
			tmp[1] = _mm_load_si128(reinterpret_cast<const __m128i*>(aVector.elements + 4));
			return tmp;
		}
		...
		static ANVIL_STRONG_INLINE Vector<T, S> ANVIL_VECTOR_CALL store(const register type aValue) throw() {
			...
			_mm_store_si128(reinterpret_cast<__m128i*>(tmp.elements + 4), aValue[1]);
			return tmp;
		}
	};

	template<>
	struct OptimisedVector<int32_t, 16> {
		typedef float T;
		enum { S = 16, optimised = 1 };
		typedef std::array<__m128i, 4> type;

		static ANVIL_STRONG_INLINE type ANVIL_VECTOR_CALL load(const Vector<T, S> aVector) throw() {
			type tmp;
			tmp[0] = _mm_load_si128(reinterpret_cast<const __m128i*>(aVector.elements));
			tmp[1] = _mm_load_si128(reinterpret_cast<const __m128i*>(aVector.elements + 4));
			tmp[2] = _mm_load_si128(reinterpret_cast<const __m128i*>(aVector.elements + 8));
			tmp[3] = _mm_load_si128(reinterpret_cast<const __m128i*>(aVector.elements + 12));
			return tmp;
		}
		...
		static ANVIL_STRONG_INLINE Vector<T, S> ANVIL_VECTOR_CALL store(const register type aValue) throw() {
			...
			_mm_store_si128(reinterpret_cast<__m128i*>(tmp.elements), aValue[0]);
			_mm_store_si128(reinterpret_cast<__m128i*>(tmp.elements + 4), aValue[1]);
			_mm_store_si128(reinterpret_cast<__m128i*>(tmp.elements + 8), aValue[2]);
			_mm_store_si128(reinterpret_cast<__m128i*>(tmp.elements + 12), aValue[3]);
			return tmp;
		}
	};
#endif
...
static ANVIL_STRONG_INLINE __m128i ANVIL_VECTOR_CALL _mm_not_epi32(const register __m128i a) throw() {
	return _mm_xor_si128(a, _mm_cmpeq_epi32(a, a));
}

static ANVIL_STRONG_INLINE __m128i ANVIL_VECTOR_CALL _mm_return_epi32(const register __m128i a) throw() {
	return a;
}

ANVIL_VECTOR_OP1(VEC_NOT, _mm_not_epi32)
ANVIL_VECTOR_OP1(VEC_FLOOR, _mm_return_epi32)
ANVIL_VECTOR_OP1(VEC_CEIL, _mm_return_epi32)
ANVIL_VECTOR_OP1(VEC_ROUND, _mm_return_epi32)

#ifdef ANVIL_SSSE_3
	ANVIL_VECTOR_OP1(VEC_ABS, _mm_abs_epi32)
#endif
...
#define ANVIL_VECTOR_OP2(ID)\
	...
	static ANVIL_STRONG_INLINE __m128i ANVIL_VECTOR_CALL _mm_mul_epi32(const register __m128i a, const register __m128i b) throw() {
		const register __m128i xmm0 = _mm_mullo_epi16(a, b);
		const register __m128i xmm1 = _mm_mullo_epi16(_mm_srli_epi32(a, 8), _mm_srli_epi32(b, 8));
		return _mm_or_si128(_mm_slli_epi32(xmm1, 8), _mm_srli_epi32(_mm_slli_epi32(xmm0, 8), 8));
	}
#endif

static ANVIL_STRONG_INLINE __m128i ANVIL_VECTOR_CALL _mm_cmpneq_epi32(const register __m128i a, const register __m128i b) throw() {
	return _mm_not_epi32(_mm_cmpeq_epi32(a, b));
}

static ANVIL_STRONG_INLINE __m128i ANVIL_VECTOR_CALL _mm_cmple_epi32(const register __m128i a, const register __m128i b) throw() {
	return _mm_not_epi32(_mm_cmpgt_epi32(a, b));
}

static ANVIL_STRONG_INLINE __m128i ANVIL_VECTOR_CALL _mm_cmpge_epi32(const register __m128i a, const register __m128i b) throw() {
	return _mm_not_epi32(_mm_cmplt_epi32(a, b));
}


ANVIL_VECTOR_OP1(VEC_ADD, _mm_add_epi32)
ANVIL_VECTOR_OP1(VEC_SUB, _mm_sub_epi32)
ANVIL_VECTOR_OP1(VEC_MUL, _mm_mul_epi32)
#ifdef ANVIL_SSE_4_1
	ANVIL_VECTOR_OP1(VEC_MIN, _mm_min_epi32)
	ANVIL_VECTOR_OP1(VEC_MAX, _mm_max_epi32)
#endif
ANVIL_VECTOR_OP1(VEC_AND, _mm_and_si128)
ANVIL_VECTOR_OP1(VEC_OR, _mm_or_si128)
ANVIL_VECTOR_OP1(VEC_XOR, _mm_xor_si128)
ANVIL_VECTOR_OP1(VEC_CMPEQ, _mm_cmpeq_epi32)
ANVIL_VECTOR_OP1(VEC_CMPNE, _mm_cmpneq_epi32)
ANVIL_VECTOR_OP1(VEC_CMPLT, _mm_cmplt_epi32)
ANVIL_VECTOR_OP1(VEC_CMPGT, _mm_cmpgt_epi32)
ANVIL_VECTOR_OP1(VEC_CMPLE, _mm_cmple_epi32)
ANVIL_VECTOR_OP1(VEC_CMPGE, _mm_cmpge_epi32)
...
#define ANVIL_VECTOR_OP2(ID)\
	...
	struct PrimativeVectorOperation<__m128i, ID, int32_t, 4> {\
		static inline __m128i ANVIL_VECTOR_CALL implementation(const register __m128i a, const register __m128i b, const register __m128i c) {\
			...
			buffer_a[0] = PrimativeVectorOperation<int32_t, ID, int32_t, 1>::implementation(buffer_a[0], buffer_b[0], buffer_c[0]);\
...
static ANVIL_STRONG_INLINE __m128i ANVIL_VECTOR_CALL _mm_fmadd_epi32(const register __m128i a, const register __m128i b, const register __m128i c) throw() {
	return _mm_add_epi32(_mm_mul_epi32(a, b), c);
}

static ANVIL_STRONG_INLINE __m128i ANVIL_VECTOR_CALL _mm_fmsub_epi32(const register __m128i a, const register __m128i b, const register __m128i c) throw() {
	return _mm_sub_epi32(_mm_mul_epi32(a, b), c);
}

ANVIL_VECTOR_OP1(VEC_FMA, _mm_fmadd_epi32)
ANVIL_VECTOR_OP1(VEC_FMS, _mm_fmsub_epi32)
