...
namespace anvil { namespace detail {
	...
	struct VectorAnd {
		...
		template<uint64_t instruction_set>
		static ANVIL_STRONG_INLINE type Execute(const type& a, const type& b) throw() {
			return OptimisedVectorAnd<T>::Execute< instruction_set>(a, b);
		}
	};
	...
	template<>
	struct OptimisedVectorAnd<float32_t> {
	...
	template<>
	struct OptimisedVectorAnd<float64_t> {
	...
	struct OptimisedVectorAnd<UnoptimisedNativeType<T,S>> {
		typedef UnoptimisedNativeType<T, S> type;

		template<uint64_t instruction_set>
		static ANVIL_STRONG_INLINE type Execute(type a, const type& b) throw() {
			for (size_t i = 0u; i < S; ++i) a[i] = OptimisedVectorAnd<T>::Execute<instruction_set>(a[i], b[i]);
		}
	};
	...
	template<uint64_t instruction_set>
	static ANVIL_STRONG_INLINE void OptimisedAnd128(const void* a, const void* b, void* dst) {
	...
	template<uint64_t instruction_set>
	static ANVIL_STRONG_INLINE void OptimisedAnd256(const void* a, const void* b, void* dst) {
		if constexpr ((instruction_set & ASM_AVX) != 0ull) {
			...
			OptimisedAnd128<instruction_set>(a, b, dst);
			OptimisedAnd128<instruction_set>(static_cast<const uint8_t*>(a) + 16u, static_cast<const uint8_t*>(b) + 16u, static_cast<uint8_t*>(dst) + 16u);
		}
	}

	template<uint64_t instruction_set>
	static ANVIL_STRONG_INLINE void OptimisedAnd512(const void* a, const void* b, void* dst) {
		if constexpr ((instruction_set & ASM_AVX512F) != 0ull) {
			...
			OptimisedAnd256<instruction_set>(a, b, dst);
			OptimisedAnd256<instruction_set>(static_cast<const uint8_t*>(a) + 32u, static_cast<const uint8_t*>(b) + 32u, static_cast<uint8_t*>(dst) + 32u);
		}
	}

	template<>
	struct OptimisedVectorAnd<__m128> {
		typedef __m128 type;
		...
		static ANVIL_STRONG_INLINE type Execute(type a, type b) throw() {
			if constexpr ((instruction_set & ASM_SSE) != 0ull) {
				...
				OptimisedAnd128<instruction_set>(&a, &b, &a);
				return a;
			}
		}
	};
	...
	template<>
	struct OptimisedVectorAnd<__m128i> {
	...
	template<>
	struct OptimisedVectorAnd<__m128d> {
	...
	template<>
	struct OptimisedVectorAnd<__m256> {
		typedef __m256 type;
		...
		static ANVIL_STRONG_INLINE type Execute(type a, type b) throw() {
			if constexpr ((instruction_set & ASM_AVX) != 0ull) {
				...
				OptimisedAnd256<instruction_set>(&a, &b, &a);
				return a;
			}
		}
	};
	...
	template<>
	struct OptimisedVectorAnd<__m256i> {
	...
	template<>
	struct OptimisedVectorAnd<__m256d> {
	...
	template<>
	struct OptimisedVectorAnd<__m512> {
		typedef __m512 type;
		...
		static ANVIL_STRONG_INLINE type Execute(type a, type b) throw() {
			if constexpr ((instruction_set & ASM_AVX512F) != 0ull) {
				...
				OptimisedAnd512<instruction_set>(&a, &b, &a);
				return a;
			}
		}
	};
	...
	template<>
	struct OptimisedVectorAnd<__m512i> {
	...
	template<>
	struct OptimisedVectorAnd<__m512d> {
	...
	struct VectorAnd<detail::BasicVector<T, size>> {
		...
		template<uint64_t instruction_set>
		static ANVIL_STRONG_INLINE type Execute(type a, const type& b) throw() {
			a.native = OptimisedVectorAnd<type::native_t>::Execute<instruction_set>(a.native, b.native);
			return a;
		}
	};
}}
