...
namespace anvil { namespace detail {
	template<class T>
	using VectorSub = VectorArith<ANVIL_VECTOR_ARITH_SUB, T>;

	template<class T>
	using VectorSubs = VectorArith<ANVIL_VECTOR_ARITH_SUBS, T>;
}}
...
namespace anvil {
	...
	template<uint64_t instruction_set = ASM_MINIMUM, class T>
	static inline T VectorSub(const T& a, const T& b, const T& src, const uint64_t mask) throw() {
	...
	template<uint64_t instruction_set = ASM_MINIMUM, class T>
	static inline T VectorSubs(const T& a, const T& b) throw() {
		return detail::VectorSubs<T>::Execute<instruction_set>(a, b);
	}
	...
	template<uint64_t mask, uint64_t instruction_set = ASM_MINIMUM, class T>
	static inline T VectorSubs(const T& a, const T& b, const T& src) throw() {
		return detail::VectorSubs<T>::ExecuteCompiletimeMask<mask, instruction_set>(a, b, src);
	}
}
...
ANVIL_STRONG_INLINE anvil::detail::BasicVector<T, size> operator-(const anvil::detail::BasicVector<T, size>& a, const anvil::detail::BasicVector<T, size>& b) throw() {
	return anvil::VectorSub<anvil::ASM_MINIMUM, anvil::detail::BasicVector<T, size>>(a, b);
}
