...
namespace anvil {
	...
	static ANVIL_STRONG_INLINE uint8_t ANVIL_CALL bswap(uint8_t aValue) throw() {
		return aValue;
	}

	// 16-bit (unsigned)

	static ANVIL_STRONG_INLINE uint16_t ANVIL_CALL bswap(uint16_t aValue) throw() {
		#if ANVIL_CPU_ARCHITECTURE == ANVIL_CPU_X86 || ANVIL_CPU_ARCHITECTURE == ANVIL_CPU_X86_64
			#if ANVIL_COMPILER == ANVIL_MSVC
				return _byteswap_ushort(aValue);
			#else
				return numeric_reinterpret_cast<uint16_t>(__builtin_bswap16(numeric_reinterpret_cast<int16_t>(aValue)));
			#endif
		#else
	...
	static ANVIL_STRONG_INLINE uint32_t ANVIL_CALL bswap(uint32_t aValue) throw() {
		#if ANVIL_CPU_ARCHITECTURE == ANVIL_CPU_X86 || ANVIL_CPU_ARCHITECTURE == ANVIL_CPU_X86_64
			#if ANVIL_COMPILER == ANVIL_MSVC
				return _byteswap_ulong(aValue);
			#else
				return numeric_reinterpret_cast<uint32_t>(__builtin_bswap32(numeric_reinterpret_cast<int32_t>(aValue)));
			#endif
		#else
	...
	static ANVIL_STRONG_INLINE uint64_t ANVIL_CALL bswap(uint64_t aValue) throw() {
		#if ANVIL_CPU_ARCHITECTURE == ANVIL_CPU_X86 || ANVIL_CPU_ARCHITECTURE == ANVIL_CPU_X86_64
			#if ANVIL_COMPILER == ANVIL_MSVC
				return _byteswap_uint64(aValue);
			#else
				return numeric_reinterpret_cast<uint64_t>(__builtin_bswap64(numeric_reinterpret_cast<int64_t>(aValue)));
			#endif
		#else
	...
	static ANVIL_STRONG_INLINE int8_t ANVIL_CALL bswap(int8_t aValue) throw() {
		return aValue;
	}

	static ANVIL_STRONG_INLINE int16_t ANVIL_CALL bswap(int16_t aValue) throw() {
		return numeric_reinterpret_cast<int16_t>(bswap(numeric_reinterpret_cast<uint16_t>(aValue)));
	}

	static ANVIL_STRONG_INLINE int32_t ANVIL_CALL bswap(int32_t aValue) throw() {
		return numeric_reinterpret_cast<int32_t>(bswap(numeric_reinterpret_cast<uint32_t>(aValue)));
	}

	static ANVIL_STRONG_INLINE int64_t ANVIL_CALL bswap(int64_t aValue) throw() {
		return numeric_reinterpret_cast<int64_t>(bswap(numeric_reinterpret_cast<uint64_t>(aValue)));
	}

	// other types

	static ANVIL_STRONG_INLINE float ANVIL_CALL bswap(float aValue) throw() {
		return numeric_reinterpret_cast<float>(bswap(numeric_reinterpret_cast<uint32_t>(aValue)));
	}

	static ANVIL_STRONG_INLINE double ANVIL_CALL bswap(double aValue) throw() {
