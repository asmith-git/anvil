...
namespace anvil {
	...
	*/
	...
	public:
		...
		LocalPointer& operator=(this_t&&) = delete;
		LocalPointer& operator=(const this_t&) = delete;
		...
		inline std::enable_if<std::is_move_assignable<T2>::value && ! std::is_abstract<T2>::value, element_type>::type release() {
		...
		template<class T2 = element_type>
		inline std::enable_if<std::is_abstract<T2>::value || !std::is_move_assignable<T2>::value, void>::type release() {
			Deallocate();
		}
