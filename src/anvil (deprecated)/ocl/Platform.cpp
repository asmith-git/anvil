...
namespace anvil { namespace ocl {
	...
	void* ANVIL_CALL Platform::getInfo(cl_platform_info aName) const throw() {
		...
		std::cerr << getErrorName(error) << " <- clGetPlatformInfo (" << mHandle.platform << ", " << info << ", " << PLATFORM_INFO_BUFFER_SIZE << ", " <<
...
#ifdef ANVIL_LOG_OCL
		std::cerr << getErrorName(error) << " <- clGetDeviceIDs (" << mHandle.platform << ", " << aType << ", " << 0 <<
...
#ifdef ANVIL_LOG_OCL
		std::cerr << getErrorName(error) << " <- clGetDeviceIDs (" << mHandle.platform << ", " << aType << ", " << count <<
...
#ifdef ANVIL_LOG_OCL
		std::cerr << getErrorName(error) << " <- clGetPlatformIDs (" << 0 << ", " << "NULL" << ", " << (void*)&count << ")" << std::endl;
#endif
		if (error != CL_SUCCESS) {
...
#ifdef ANVIL_LOG_OCL
		std::cerr << getErrorName(error) << " <- clGetPlatformIDs (" << count << ", " << (void*)platformIDs << ", " << "NULL" << ")" << std::endl;
#endif
		if (error != CL_SUCCESS) {
			oclError("clGetPlatformIDs", error);
			return std::vector<Platform>();
